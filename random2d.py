...
#random movement of agent in 2d environment
"""
names:
    ...
    def move(self):
        ...
        if per==1:
            ...
            if self.dir==0:
               ...
               self.loc=(self.loc[0] + 1,self.loc[1] +0)
            elif self.dir==1:
               self.loc=(self.loc[0] - 1,self.loc[1] +0)
            elif self.dir==2:
               self.loc=(self.loc[0] + 0,self.loc[1] +1)
            else:
               self.loc=(self.loc[0] + 0,self.loc[1] -1)
...
class Environment2d:
    ...
    def percept(self, dir):
        ...
        if dir==0:
               cur_loc=(self.iloc[0] + self.agent.loc[0] + 1,self.iloc[1] + self.agent.loc[1] +0)
        elif dir==1:
               cur_loc=(self.iloc[0] + self.agent.loc[0] - 1,self.iloc[1] + self.agent.loc[1] +0)
        elif dir==2:
               cur_loc=(self.iloc[0] + self.agent.loc[0] +0,self.iloc[1] + self.agent.loc[1] +1)
        else:
               cur_loc=(self.iloc[0] + self.agent.loc[0] +0,self.iloc[1] + self.agent.loc[1] -1)

        print "dir",dir
        print "current_loc",cur_loc
        
        if cur_loc==(self.l,self.l):
                return 2
        if(cur_loc[0] < 0 or cur_loc[1]<0 or cur_loc[0] > self.l or cur_loc[1]>self.l):
            return 0
